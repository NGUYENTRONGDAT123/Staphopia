...
(function($){
    ...
    if(href.indexOf('/refseq/functionalelements/') > -1){
        // For RefSeq Functional Elements Page
        append_feedback_button('LGT9ZVB');
    }else if(href.indexOf('/refseq/refseq_select/') > -1){
        // For RefSeq Select Page
        append_feedback_button('9F52F2M');
    }else if(href.indexOf('/refseq/MANE/') > -1){
        // For RefSeq MANE Page
        append_feedback_button('9F5S7R5');
    }
    function append_feedback_button(research_code){
        var app = $("meta[name='subsite']").attr('content');
        var pdid = $("meta[name='ncbi_pdid']").attr('content');
        var session = $.cookie("ncbi_sid");
        var ref = encodeURIComponent(href);
        var url = 'https://www.research.net/r/' + research_code + '?a=' + app + '&from=' + ref + '&p=' + pdid + '&s=' + session;
        $("body").append('<a class="ncbi-feedback-button" data-surveycode="surveycode" href="' + url + 
            '" ref="linksrc=feedback_btn" rel="nofollow" target="_blank">Feedback</a>');
    }
})(jQuery);
;
(function($){
