...
export default function BubbleChart (props) {
  ...
  //pack data
  function pack () {
    return d3.pack ().size ([500 - 2, 500 - 2]).padding (3);
  }
  ...
  const drawChart = useCallback (
    () => {
      ...
      //   .style("fill", "#69b3a2");
      //design the node
      const node = svg.selectAll ('circle').data (root.descendants ()).join (
        enter => {
          return enter
            .append ('circle')
      ...
      node
        .on ('mouseover', function (event, d) {
          ...
          if (!d.children) {
            d3
              .selectAll ('._' + d.data.name.replace ('.csv', ''))
              .attr ('stroke', '#000')
        ...
        .on ('click', (event, d) => {
          ...
          if (d.depth === 2) {
            ...
            if (preClick !== null) {
              d3
                .selectAll ('._' + preClick)
                .attr ('class', 'node node--leaf _' + preClick);
            }
            d3
              .selectAll ('._' + d.data.name.replace ('.csv', ''))
              .attr (
      ...
      const label = svg
        ...
        .text (
          d => (!d.children ? d.data.name.replace ('.csv', '') : d.data.name)
        );
      label.exit ().remove ();
